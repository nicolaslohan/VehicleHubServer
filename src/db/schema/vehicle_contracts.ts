import { pgTable, varchar, timestamp, bigserial, boolean, bigint, numeric } from 'drizzle-orm/pg-core'
import { companies } from './companies'

export const vehicle_contracts = pgTable('vehicle_contracts', {
    id: bigserial('id', { mode: 'number' }).primaryKey(),
    company_id: bigint('company_id', { mode: 'number' }).references(() => companies.id).notNull(),
    milleage_limit: bigint('milleage_limit', { mode: 'number' }).notNull(),
    contract_total_amount: numeric('contract_total_amount', { precision: 10, scale: 2 }).notNull(),
