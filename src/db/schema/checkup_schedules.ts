...
export const checkup_schedules = pgTable('checkup_schedules', {
    ...
    vehicle_id: bigint('vehicle_id', { mode: 'number' }).references(() => vehicles.id).notNull(),
    vehicle_action_id: bigint('vehicle_action_id', { mode: 'number' }).references(() => vehicle_actions.id),
